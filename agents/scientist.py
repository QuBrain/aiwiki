        ...
        for i in range(random.randint(2, 4)):
            section_title = (
                ...
                if i == 0
                    ...
                    if i == 1
                    else (f"Current Research in {topic}" if i == 2 else "Future Directions")
                )
            )
            section_body = f"The scientific study of {topic} has revealed remarkable insights. "
            section_body += "Researchers continue to push the boundaries of knowledge. "
            section_body += "Experimental evidence supports the theoretical framework. "
            section_body += "Further investigation promises to unlock new understanding."
